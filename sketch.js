...
planet=createSprite(width/2,height/2);
 ...
 planet.scale=0.6;
  ...
  redj=createSprite(200,450)
  redj.addImage(redimage)
  redj.scale=0.1;
  redj.depth=2;
  redj.visible=false;
 ...
 start=createSprite(300,300);
  ...
  back8.scale=0.2
       ...
       backgroundr=createSprite(300,300);
    backgroundr.addImage(backgroundimage);
    backgroundr.depth=0.1
  backgroundr.visible=false;
  backgroundr.velocityX=-4
  
 gun1r=createSprite(2600,60,10,10);
gun1r.shapeColor="orange";
  gun1r.visible=false;
  
bulletgroup1r=new Group();
}
...
text("In which birds have to destroy",10,330);
  ...
  if(gamestate===REDPLAY){
    ...
    redj.collide(rblock1);
    
    var redroof=createSprite(300,30,10000,20);
    redj.collide(redroof);
    
    var rblock2=createSprite(1000,400,50,200);
    redj.collide(rblock2);
    
    var rblock3=createSprite(1300,400,200,270);
    redj.collide(rblock3);
    rblock3.depth=3

    
    var rblock4=createSprite(400,300,70,10);
    redj.collide(rblock4);
 ...
 rlazer1.depth=0.2
    ...
    redj.collide(rblock6);
    
    var rblock7=createSprite(2500,300,500,50);
    redj.collide(rblock7);
    
    var rblock8=createSprite(3000,600,1000,100);
    redj.collide(rblock8);

    var spin=createSprite(2600,500,10,10);
    spin.addImage(spinimage);
    spin.velocityY=spin.velocityY+12;
    spin.debug=true;
    spin.setCollider("circle",0,0,30);
    spin.collide(rblock8)
    spin.visible=false;
    
    if(redj.isTouching(spin)){
      gamestate=END;
    }
    
    var extrablockred=createSprite(3000,500,900,5);
    extrablockred.shapeColor="yellow"
    
    if(redj.isTouching(extrablockred)){
      spin.velocityX=20
      spin.visible=true;
    }
    
    
    var rblock9=createSprite(4000,300,400,20);
    redj.collide(rblock9);
 
    if(bulletgroup1r.isTouching(rblock7)){
bulletgroup1r.destroyEach();
    }
    if(bulletgroup1r.isTouching(redj)){
gamestate=END;
    }
    
    redj.velocityY=redj.velocityY+2
...
function saber(){
  var lightsaber=createSprite(redj.x+55,redj.y+10,50,3);
  lightsaber.life=1;
  lightsaber.shapeColor="cyan";
}
function bullet(){
  if(World.frameCount%10===0){
var b=createSprite(200,200,5,5);
    b.x=gun1r.x;
    b.y=gun1r.y
  b.velocityY=1;
  b.shapeColor="red";
